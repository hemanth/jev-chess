 ...
 */
class SimulatedTypeSafeClient {
    isLive = false;
    async systemOne(request) {
        const answers = {};
        const stateStr = typeof request.state === "string"
            ? request.state
        ...
        for (const [id, q] of Object.entries(request.questions)) {
            ...
            if (q.type === "noul") {
                ...
                let selected = "none";
                const probabilities = {};
                const userIntent = typeof request.state === "object" && request.state !== null && "user_intent" in request.state
                    ? String(request.state.user_intent).toLowerCase()
                ...
                for (const k of keys) {
                    ...
                    if (userIntent.includes("knight") && (desc.includes("knight") || kLower.startsWith("n")))
                        s += 10;
                    if (userIntent.includes("bishop") && (desc.includes("bishop") || kLower.startsWith("b")))
                        s += 10;
                    if (userIntent.includes("queen's pawn") || userIntent.includes("d-pawn")) {
                        if (userIntent.includes("two square") && (kLower === "d4" || desc.includes("d2 to d4")))
                            s += 20;
                        else if (kLower === "d3" || desc.includes("d2 to d3"))
                            s += 15;
                    }
                    if (userIntent.includes("king's pawn") || userIntent.includes("e-pawn")) {
                        if (userIntent.includes("two square") && (kLower === "e4" || desc.includes("e2 to e4")))
                            s += 20;
                        else if (kLower === "e3" || desc.includes("e2 to e3"))
                            s += 15;
                    }
                    if (userIntent.includes("castle") && (desc.includes("castle") || kLower.includes("o-o")))
                        s += 25;
                    if (userIntent.includes("center") && (kLower === "d4" || kLower === "e4" || kLower === "nf3" || kLower === "nc3"))
                        s += 6;
                    if (userIntent.includes("attack") && (desc.includes("captures") || kLower === "nf3"))
                        s += 5;
                    // Strategic theme matching
                    if (userIntent.includes("strategic") || stateStr.includes("strategic_theme")) {
                        if (stateStr.includes("o-o") && kLower === "prophylaxis")
                            s += 30;
                        if (stateStr.includes("c3") && kLower === "pawn_break")
                            s += 30;
                        if (stateStr.includes("bxf7+") && kLower === "tactical_strike")
                            s += 35;
                    }
                    // Mistake archetype diagnostics
                    if (stateStr.includes("??") || stateStr.includes("blunder")) {
                        if (kLower === "king_safety_negligence")
                            s += 40;
                        if (kLower === "tactical_blindness")
                            s += 25;
                    }
                    if (s > bestScore) {
                        bestScore = s;
...
class LiveTypeSafeClientWrapper {
    isLive = true;
    client;
    constructor(apiKey) {
...
//# sourceMappingURL=typeSafeClient.js.map